	...
	if(cadena != NULL && largoCadena > 0)
	{
		for(int i = 0; i < largoCadena; i++)
		{
			if(cadena[i] == '!' || cadena[i] == '#' || cadena[i] == '$' || cadena[i] == '@' ||
			   ...
			   cadena[i] == '[' || cadena[i] == ']' || cadena[i] == ':' || cadena[i] == '`' ||
			   cadena[i] == ';' || cadena[i] == '"' || cadena[i] == '<' || cadena[i] == '>' ||
			   cadena[i] == '?' || cadena[i] == '~' || isspace(cadena[i]) || cadena[largoCadena - 1] == '.')
			{
				exito = -1;
				break;
			}
...
int utn_getPathLimited(char* pString, char* mensaje, char* mensajeError, int maximosCaracteres, int reintentosMaximos)
{
	int exito = -1;
	char auxPath[maximosCaracteres];

	printf(mensaje);
	fflush(stdin);
	gets(auxPath);

	while((strlen(auxPath) > maximosCaracteres || isStringPath(auxPath) == -1) && reintentosMaximos > 0)
	{
		reintentosMaximos--;
		if(reintentosMaximos != 0)
		{
...
int utn_StringToUpper(char* cadena, char* cadenaToUpper)
{
	int exito = -1;
	int largoCadena = strlen(cadena);
